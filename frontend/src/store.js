...
// contribution count, 24h access window, add allowance, and human-verified
// flag.
...
// Whether the viewer has opted to include people who have died in the
// rankings and matchups. Default off (the living only). Persisted locally so
// the choice holds across the leaderboard and the voting view and survives a
// reload — it's a display preference, never sent to or stored by the server
// beyond the per-request query flag.
...
// only" tier. Like the deceased filter, these scope *which* figures are drawn
// and ranked — never a separate ranking — and are display preferences
// persisted locally, sent only as per-request query flags.
...
export const poolRegion = ref(
  REGIONS.includes(readLS(REGION_KEY, '')) ? readLS(REGION_KEY, '') : '',
)
...
// The finer geographic scope (docs/10 §4): a single country — the same axis
// as poolRegion at a sharper zoom, so the picker offers one *or* the other
// (choosing a country clears the region and vice versa). The value is the
// country's English label, exactly what the server stores and filters on.
// Persisted locally and sent only as a per-request query flag, like the rest
// of the pool.
...
// browser's volunteered locale (fr-FR → France) — never IP (docs/10 §6).
// The value is the Wikidata English label the backend stores, so an exact
// match leans the draw toward that country; an unmapped locale (or a label
// not in the pool) just yields no lean and the continent suggestion still
// applies.
// The map is small and best-effort — the visitor can always pick a country
// explicitly.
const COUNTRY_BY_CC = {
...
// poolCountry). Seeded from the locale when never set, so the silent majority
// who never open the picker still get a local-leaning draw; '' means no lean.
// Never inferred from IP; persisted locally and sent only as a per-request
// query flag.
...
// The visitor's home region (docs/10 §4): the continent they follow most
// closely, chosen once at onboarding. Unlike poolRegion (a *strict* filter),
// this is a *soft* bias — the server leans the matchup draw toward this
// region without excluding anyone, so the cross-region discovery slot still
// bridges the pools. '' is the whole world (no lean). Never inferred from IP
// (docs/10 §6); persisted locally and sent only as a per-request query flag.
export const homeRegion = ref(
  REGIONS.includes(readLS(HOME_KEY, '')) ? readLS(HOME_KEY, '') : '',
)
...
// suggestRegion proposes a home region from the browser's *volunteered*
// language (navigator.languages) — never from IP (docs/10 §6) — to
// pre-highlight a chip in the onboarding step. A region subtag is the
// strong signal (pt-BR → South America, en-AU → Oceania); a bare language
// is a weak fallback; anything global or ambiguous (en, es, ar) yields '' so
// we don't presume. The visitor always confirms, so this only has to be a
// sensible default, not exact.
const REGION_BY_LOCALE = {
  ...
  'fr-ca': 'North America', 'fr-fr': 'Europe',
  'fr-be': 'Europe', 'fr-ch': 'Europe',
...
const REGION_BY_LANG = {
  ...
  ro: 'Europe', hu: 'Europe', fi: 'Europe', da: 'Europe', no: 'Europe',
  fr: 'Europe',
...
// Recall (docs/11 §2) is asked once per pool scope; we remember which scopes
// the visitor has already answered, locally only (never sent — like the
// welcome flag).
...
export function applyVote(res) {
  if (typeof res.contributions === 'number') {
    me.contributions = res.contributions
  }

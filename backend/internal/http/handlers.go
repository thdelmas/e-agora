...
// matchup returns two distinct active subjects, localized per R9. By default
// the pair is drawn from the living; ?includeDeceased opts historical figures
// back in so a visitor can compare them against the living
// (docs/05-ranking.md §Filtering the deceased).
func (h *handlers) matchup(w http.ResponseWriter, r *http.Request) {
	...
	// The home region/country (docs/10 §4) are *soft* draw biases — the
	// visitor's home continent and country, leaning the recognition draw
	// toward figures they're likeliest to know without filtering anyone out.
	// Distinct from the PoolPicker's strict ?region/?country (poolFrom); empty
	// values add no bias.
	home := strings.TrimSpace(r.URL.Query().Get("home"))
	homeCountry := strings.TrimSpace(r.URL.Query().Get("homeCountry"))
	pair, err := h.store.RandomPair(
		r.Context(), visitor, home, homeCountry, reco, h.poolFrom(r),
	)
	if errors.Is(err, store.ErrPoolTooSmall) {
		writeError(w, http.StatusConflict, "pool_too_small",
	...
	if err != nil {
		...
		writeError(w, http.StatusInternalServerError, "internal",
			"Could not load a matchup.")
		return
	}
	a, b := pair[0], pair[1]

	display, fellBack := lang.Resolve(
		visitor, h.cfg.FallbackLang, a.AvailableLangs, b.AvailableLangs,
	)
...
func (h *handlers) vote(w http.ResponseWriter, r *http.Request) {
	...
	if sess.HumanVerifiedUntil == nil ||
		...
		writeError(w, http.StatusForbidden, "human_check_required",
			"Prove you're human before voting.")
		return
	}

	var req voteRequest
	dec := json.NewDecoder(http.MaxBytesReader(w, r.Body, 4096))
	if err := dec.Decode(&req); err != nil || req.WinnerID == 0 ||
		req.LoserID == 0 || req.WinnerID == req.LoserID {
		writeError(w, http.StatusBadRequest, "invalid_matchup",
			"Provide distinct winnerId and loserId.")
		return
	}

	res, err := h.store.RecordVote(
		r.Context(), sess.ID, req.WinnerID, req.LoserID,
	)
	if errors.Is(err, store.ErrInvalidMatchup) {
		writeError(w, http.StatusBadRequest, "invalid_matchup",
			"Those subjects aren't a valid, active pair.")
		return
	}
	if err != nil {
...
// rolling).
func (h *handlers) ensureAccessToken(
...
// localize returns display content for a subject in displayLang, lazily
// fetching and caching a missing translation, falling back to English
// (docs/06 §Step 4).
func (h *handlers) localize(
	ctx context.Context, subj model.Subject, displayLang string,
) model.Translation {
	if tr, found, err := h.store.Translation(
		ctx, subj.ID, displayLang,
	); err == nil && found {
		return tr
	}
	if displayLang != "en" {
	...
	return model.Translation{
		...
		WikipediaURL: "https://en.wikipedia.org/wiki/" +
			strings.ReplaceAll(subj.CanonicalName, " ", "_"),
	}
}

// ensureExtract backfills a missing lead paragraph for a subject shown in a
// matchup. Rows seeded before extracts were stored (and lazily-cached
// languages) have a description but no extract; this best-effort refetch fills
// it in the language already resolved, so the card gets its inline paragraph
// and the row is upserted for next time. Only the matchup needs this (≤2
// calls, decaying to zero as the pool's extracts fill); the leaderboard
// doesn't show extracts.
func (h *handlers) ensureExtract(
	ctx context.Context, subj model.Subject, tr model.Translation,
) model.Translation {
	if tr.Extract != "" || tr.Lang == "" {
		return tr
	}
	if fresh, ok := h.fetchTranslation(
		ctx, subj, tr.Lang,
	); ok && fresh.Extract != "" {
		return fresh
	}
...
func (h *handlers) fetchTranslation(
	ctx context.Context, subj model.Subject, displayLang string,
) (model.Translation, bool) {
	...
	if url == "" {
		url = "https://" + displayLang + ".wikipedia.org/wiki/" +
	...
	if err := h.store.UpsertTranslation(ctx, subj.ID, displayLang, name,
		sum.Description, sum.Extract, sum.ImageURL, url); err != nil {
		h.logger.Warn("cache translation", "qid", subj.WikidataID,
			"lang", displayLang, "err", err)
	}
	return model.Translation{
		SubjectID: subj.ID, Lang: displayLang, Name: name,
		Description: sum.Description, Extract: sum.Extract,
		ImageURL: sum.ImageURL, WikipediaURL: url,
	}, true
}

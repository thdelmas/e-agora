...
// recall searches active subjects by name for the recall type-ahead
// (docs/11 §2). Public-with-session like the matchup; an empty q returns
// nothing.
func (h *handlers) recall(w http.ResponseWriter, r *http.Request) {
	q := strings.TrimSpace(r.URL.Query().Get("q"))
	if q == "" {
		writeJSON(w, http.StatusOK,
			map[string]any{"results": []store.SubjectRef{}})
		return
	}
...
// proposalRequest names the recalled figure: subjectId for someone already in
// the pool, or url (a Wikipedia URL) for someone not yet seeded — the latter
// is ingested on first recall (docs/11 §3).
...
// proposal records one recall of a subject for the active pool (docs/11 §2),
// the belonging signal. Rate-limited but, unlike a vote, *not* humanity-gated:
// recall is the visitor's first interaction (pool entry, before any vote), so
// gating it would wall off the casual visitor the recognition redesign exists
// to serve. The abuse surface is low — the smoothed score moves n and N
// together, so one session is near-neutral and shifting belonging needs many
// distinct sessions (which the humanity check on *votes* still discourages).
// The pool scope comes from the same query params as the matchup/leaderboard
// (poolFrom → PoolKey); subject in the body.
func (h *handlers) proposal(w http.ResponseWriter, r *http.Request) {
	...
	if subjectID == 0 {
		...
		if strings.TrimSpace(req.URL) == "" {
			writeError(w, http.StatusBadRequest, "invalid_request",
				"Provide a subjectId or url to propose.")
			return
		}
	...
	if errors.Is(err, store.ErrInvalidProposal) {
		writeError(w, http.StatusBadRequest, "invalid_proposal",
	...
	if err != nil {
		...
		writeError(w, http.StatusInternalServerError, "internal",
			"Could not record your proposal.")
		return
	}

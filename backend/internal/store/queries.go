...
type RecoParams struct {
	...
	Country       float64 // flat boost for a subject in the visitor's home country — a soft bias, finer/stronger than Region
...
type Pool struct {
	...
	Country         string  // region axis (finer): "" = any, else the country's English label, e.g. "France"
...
//
// homeRegion / homeCountry are the visitor's home continent and country (docs/10
// §4) — *soft* biases, distinct from pool.Continent / pool.Country's hard filters.
// A subject in the home region gets a flat p.Region added to R, and one in the
// home country a flat p.Country (finer, so weighted stronger), lifting modest
// *local* figures the visitor knows without excluding anyone: the recognition
// draw leans home, yet the discovery challenger (coverage-biased, geo-blind) still
// reaches across continents, so the comparison graph stays connected and the one
// Glicko scale comparable. An empty home value (never chosen, or "whole world")
// adds nothing.
//
